...
module Continuation = struct
  ...
  let make ~flags ~stream_id ~payload =
    if stream_id = 0 then
      ...
      let fragment =
        Bytestring.of_string (Bitstring.string_of_bitstring payload)
      in
      Ok { stream_id; fragment; end_headers = has_headers_bit flags }

  let[@tail_mod_cons] rec parts t =
    let length = Bytestring.length t.fragment in
    if length <= max_frame_size then
      let data =
        Bytestring.to_string t.fragment |> Bitstring.bitstring_of_string
      in
      [ (frame_type, flags [ 2 ], t.stream_id, data) ]
    else
      let this_frame =
        let sub = Bytestring.sub ~off:0 ~len:max_frame_size t.fragment in
        Bytestring.to_string sub |> Bitstring.bitstring_of_string
      in
      let rest =
        let len = Bytestring.length t.fragment - max_frame_size in
        Bytestring.sub ~off:max_frame_size ~len t.fragment
      in
...
module Data = struct
  ...
  let padded_data ~flags ~stream_id ~payload =
    ...
    | {| padding : 8 ; rest : -1 : string |} when String.length rest >= padding
      ->
        ...
        let data = Bytestring.of_string data in
  ...
  let make ~flags ~stream_id ~payload =
    if stream_id = 0 then
      ...
      let data = Bytestring.of_string (Bitstring.string_of_bitstring payload) in
      Ok { stream_id; data; end_stream = has_end_stream_bit flags; flags }

  let[@tail_mod_cons] rec parts t =
    let length = Bytestring.length t.data in
    if length <= max_frame_size then
      let flags = flags (if t.end_stream then [ 0 ] else []) in
      let data = Bytestring.to_string t.data |> Bitstring.bitstring_of_string in
      [ (frame_type, flags, t.stream_id, data) ]
    else
      let this_frame =
        let sub = Bytestring.sub ~off:0 ~len:max_frame_size t.data in
        Bytestring.to_string sub |> Bitstring.bitstring_of_string
      in
      let rest =
        let len = Bytestring.length t.data - max_frame_size in
        Bytestring.sub ~off:max_frame_size ~len t.data
      in
      let frame = (frame_type, 0x0, t.stream_id, this_frame) in
...
module Headers = struct
  ...
  let pp fmt t =
    Format.fprintf fmt
      ...
      t.weight
      (Bytestring.to_string t.fragment)
        ...
        Ok
          {
            ...
            stream_dependency = Some stream_dependency;
            weight;
            fragment = Bytestring.of_string fragment;
          }
  ...
  let parts t =
    ...
    let fragment_length = Bytestring.length t.fragment in
...
module Ping = struct
  type t = { ack : bool; payload : Bytestring.t }
  ...
  let make ~flags ~stream_id ~payload =
    ...
    | {| payload : 8 : string |} when stream_id = 0 ->
        Ok { ack = has_ack_bit flags; payload = Bytestring.of_string payload }
    | {| _ |} when stream_id != 0 ->
  ...
  let parts t =
    ...
    let payload =
      Bytestring.to_string t.payload |> Bitstring.bitstring_of_string
    in
    [ (0x6, flags, 0, payload) ]
end
        ...
        Ok
          {
            last_stream_id;
            error_code;
            debug_data = Bytestring.of_string debug_data;
          }
    | {| _ |} when stream_id != 0 ->
      ...
      ( 0x7,
        ...
        Bitstring.concat
          [
            ...
            Bitstring.bitstring_of_string (Bytestring.to_string t.debug_data);
          ] );
    ]
end
...
type t =
  ...
  | Unknown of { stream_id : stream_id; flags : int; payload : Bytestring.t }
...
let parts frame =
  ...
  | Unknown { stream_id; flags; payload } ->
      let payload =
        Bytestring.to_string payload |> Bitstring.bitstring_of_string
      in
